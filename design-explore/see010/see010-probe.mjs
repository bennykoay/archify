...
const PAGE_FN = String.raw`(function () {
  ...
  return {
    ...
    nodesRaw: nodes.map(function (n) { return { id: n.id, card: n.card, sub: n.sub ? { text: n.sub.text, box: n.sub.box } : null, title: n.title ? { text: n.title.text, box: n.title.box } : null }; }),
    edgeLabelsRaw: edgeLabels.map(function (e) { return { label: e.label, key: e.key, from: e.from, to: e.to, box: e.box }; }),
    framesRaw: boundaryRects.map(function (bd) { return { id: bd.id, label: bd.label || bd.id || bd.kind, kind: bd.kind, box: bd.box }; })
  };
})`;
let geom;
try {
...
await cleanup();
console.log(JSON.stringify({ nodes: geom.nodesRaw, edgeLabels: geom.edgeLabelsRaw, frames: geom.framesRaw }));
process.exit(0);
...
const complete = ['A1', 'A2', 'A3', 'A4', 'A5', 'A6', 'A7', 'A8', 'A9', 'A10', 'A11', 'A12', 'A13', 'O1a'].every((id) => A.some((a) => a.id === id && a.verdict));
const out = {
  tool: 'geometry-assert.mjs (SEE-005 Layer-1 ruler + SYS-003 Amendments A-D + SEE-008 A12 + SEE-009 A13/A3-feed)',
