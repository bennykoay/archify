...
test('all five renderers expose one reader-controlled visual style picker', () => {
  for (const mode of Object.keys(CASES)) {
    ...
    for (const preset of ['classic', 'signal-flow', 'blueprint', 'editorial', 'apple-hig', 'classic-original']) {
...
test('style selection synchronizes page, picker, and canonical SVG without touching geometry', () => {
  ...
  assert.match(runtime, /\['classic', 'signal-flow', 'blueprint', 'editorial', 'apple-hig', 'classic-original'\]/);
...
test('style picker follows the accessible menu-button interaction contract', () => {
  ...
  assert.match(html, /\.preset-option-swatch\.apple-hig/);
  assert.match(html, /\.preset-option-swatch\.classic-original/);
  assert.match(
    html,
...
test('same topology keeps identical canonical SVG geometry across all six styles', () => {
  const normalize = (svg) => svg.replace(/ data-preset="(?:classic|signal-flow|blueprint|editorial|apple-hig|classic-original)"/, '');
  const variants = ['classic', 'signal-flow', 'blueprint', 'editorial', 'apple-hig', 'classic-original'].map((preset) => normalize(svgBlock(render('architecture', preset))));
  ...
  assert.equal(variants[5], variants[0]);
});
