...
def summarize(out):
    ...
    for task in frozen['tasks']['tasks']:
        ...
        if both:
            ...
            br, pr = baseline['candidate_content_changes_observed'], candidate['candidate_content_changes_observed']
    ...
    gate = frozen['protocol']['screen']
    passed = bool(all_p and len(valid) >= gate['minimum_both_quality_pairs']
                  and med_seconds is not None and med_seconds >= gate['median_full_saving_seconds']
                  and med_fraction >= gate['median_full_saving_fraction']
                  and med_repairs is not None and med_repairs >= gate['median_all_observed_revision_reduction']
                  and all(p['fraction_saved'] >= -gate['max_pair_slowdown_fraction'] for p in valid))
    return {'rows': rows, 'pairs': pairs, 'all_P_quality_pass': all_p,
            'qualified_pairs': len(valid), 'median_seconds_saved': med_seconds,
            'median_fraction_saved': med_fraction, 'median_observed_content_revision_reduction': med_repairs,
            'screen_pass': passed, 'promotion_qualified': False,
            'limits': 'The screen counts all observed candidate content revisions; post-finalize revisions remain a separate diagnostic. Example exposure is inferred from coarse lexical tool access categories, not proof of comprehension. Content changes before first finalize are reported separately and must not be hidden by a zero finalized-repair count. A content change is not necessarily one model turn. Missing quality is unknown. A passing screen still requires the frozen confirmation pairs.'}
